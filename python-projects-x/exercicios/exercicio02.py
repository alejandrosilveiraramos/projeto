'''
Crie uma variável recebendo o conceito de polimorfismo com o sinal de igual.

Crie uma função de impressão usando interpolação e aplique a variável de polimorfismo antes e depois da String cabeçalho adicione quebra de linha, ao final.
Imprima no terminal o índice de repetição.

Crie um laço de repetição para imprimir a função print com a palavra FELIZ DIA DO PROGRAMADOR. a frase deve ser impressa 10 vezes.

Crie uma função de impressão usando interpolação e aplique a variável de polimorfismo antes e depois do string utilize como rodapé.

'''
# variável
def felizDiaDoProgramador():

    decoracao = "="*20

    # Função de Impressão
    print(decoracao, "CABEÇALHO", decoracao)

    # Laço de Repetição
    for c in range(1, 11):
        print(c," - FELIZ DIA DO PROGRAMADOR")

    # Função de Impressão
    print(decoracao, "RODAPÉ", decoracao)
